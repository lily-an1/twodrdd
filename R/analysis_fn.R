...
loess_2DRDD <- function( dat,
  ...
  suppressWarnings(out_loess <- purrr::map(sampdat$strap,
                                           ~ loess2DRDD(.x,
                                                        radius=mean(sd(dat$rating2),
                                                                    sd(dat$rating1))/2,
...
parametric_2DRDD <- function( dat,
  ...
  if ( !quadratic ) {
    ...
    M0 = estimatr::lm_robust(Y ~ rating1 + rating2 + rating1:rating2,
                             data = dat_0)
    M1 = estimatr::lm_robust(Y ~ rating1 + rating2 + rating1:rating2,
                             data = dat_1)
  } else {
    #Build the model matrix (for predictions, ensures proper variable ordering):
    mm <- stats::model.matrix(~ rating1 + rating2 + I(rating1^2) + I(rating2^2) + rating1:rating2,
                              data=df)

    dat_0 <- dplyr::filter(dat, T==0)
    dat_1 <- dplyr::filter(dat, T==1)

    M0 = estimatr::lm_robust(Y ~ rating1 + rating2 + I(rating1^2) + I(rating2^2) + rating1:rating2,
                             data = dat_0)
    M1 = estimatr::lm_robust(Y ~ rating1 + rating2 + I(rating1^2) + I(rating2^2) + rating1:rating2,
                             data = dat_1)
  }
...
analysis <- function( dat, cut1=0, cut2=0,
  ...
  if (include_PARA_LINEAR) {
    out_paraL <- parametric_2DRDD( dat,
                                   cut1 = cut1, cut2 = cut2,
                                   n_sentinel = n_sentinel )
  }
  ...
  if (include_PARA_QUAD) {
    out_paraQ <- parametric_2DRDD( dat,
                                   cut1 = cut1, cut2 = cut2,
                                   n_sentinel = n_sentinel,
                                   quadratic = TRUE )
  }
