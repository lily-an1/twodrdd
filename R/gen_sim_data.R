#Modified by Lily An
#from Kristin Porter's "gen.2RRDD_2013_0427.R" code shared Feb 18, 2022
...
#' function gen.2RRDD
#'
#' Generate fake data following general structure of Porter et al, as
#' adjusted by An at al (2024)
#'
#' @param n = sample size
...
#'   all params)
#' @param	data.type = "full","observed",or "none" (no data returned -
...
gen.2RRDD <- function(n,
    ...
    if (is.null(cut1.quantile) == FALSE) {
        #cut1 <- quantile(r1.obs, probs = cut1.quantile)
    ...
    if (is.null(cut2.quantile) == FALSE) {
        #cut2 <- quantile(r2.obs, probs = cut2.quantile)
...
gen_dat_sim <- function(sim = 1,
    ...
    # = quantile for cutting rating 1  } This matches KP gen RDD code
    cut2.quantile <- cut2
...
if (FALSE) {
    ...
    dat = gen_dat_sim(5, 0.8, 0.5, 0.5, 444343, n = 5000, s = 1)
    names(dat)
